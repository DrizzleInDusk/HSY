...
package com.zm.hsy.tools.ringdroid.soundfile;
...
/**
 * cheapsoundfile是每做一个“廉价”的扫描各种声音文件格式的几个子类的父类，解析尽可能以理解的高层框架结构得到每一帧的音量水平的粗略估计。每个
 子类能够：
 打开一个声音文件
 *返回采样率和帧数
 *返回每个帧的音量水平的近似值
...
public class CheapSoundFile {
    ...
    public void WriteFile(File inputFile,File outputFile, int startFrame, int numFrames)
            throws java.io.IOException {
    }
};
